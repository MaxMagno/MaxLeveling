export function XPBar({
  ...
  const glow =
    tone === "violet" ? "var(--accent)" : tone === "success" ? "var(--success)" : "var(--primary)";
  return (
    <div className="space-y-1.5">
      {label && (
        <div className="flex items-baseline justify-between text-xs">
          <span className="text-muted-foreground tracking-widest uppercase">{label}</span>
          <span className="text-neon font-display">
            {value} / {max}
          </span>
        </div>
      )}
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="relative w-14 h-14">
        ...
        <span className="absolute inset-0 grid place-items-center text-xs font-display">
          {value}
        </span>
      </div>
      ...
      <div className="mt-2 flex items-end gap-2">
        <span
          ...
          style={{
            ...
            textShadow: hot
              ? "0 0 18px color-mix(in oklab, var(--warning) 60%, transparent)"
              : undefined,
          }}
        >
          {streak}
