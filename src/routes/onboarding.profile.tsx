...
function ProfileStep() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    setProfile({
      ...
      limitations: f.limitations || undefined,
      goal: f.goal,
    });
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <header className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Estos datos calibran tu esfuerzo, seguridad y progresión. No definen lo que puedes
            lograr.
          </p>
        </header>

        <SystemMessage>
          El sistema MaxLeveling te asignará misiones diarias adaptadas a tu nivel. Toda la
          información permanece en este dispositivo.
        </SystemMessage>
        ...
        <SystemPanel eyebrow="Datos del cazador" title="Identidad y físico" neon>
          <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Sexo / género">
              <Select
                value={f.gender}
                onChange={(e) => upd("gender", e.target.value as UserGender)}
              >
            ...
            <Field label="Masa muscular (opcional)">
              <TextInput
                ...
                value={f.muscleMass}
                onChange={(e) =>
                  upd("muscleMass", e.target.value === "" ? "" : Number(e.target.value))
                }
              />
            </Field>
            <Field label="Unidad masa muscular">
              <Select
                value={f.muscleMassUnit}
